...
🜁 Rôle du dossier
La page GitHub indique clairement que ce dossier contient :
...
Les réponses présentes ici proviennent de :

/responses/raw/  
→ réponses brutes générées par les modèles

/scores/raw/  
→ scores bruts produits par les validateurs

curate_responses.py  
→ script de nettoyage, filtrage et validation
...
Ces propriétés sont explicitement listées dans la page GitHub :
...
Nettoie, filtre et valide les réponses brutes.

2. validate_dataset.py
Vérifie la conformité au schéma JSON.

3. aggregate_scores.py
Associe les scores aux réponses validées.

4. build_benchmark.py
Construit CATAR-Benchmark-v1.json à partir des réponses curated.
